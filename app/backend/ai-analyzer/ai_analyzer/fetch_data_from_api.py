        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if all(col in df.columns for col in required_company_cols):
                    df_company = df[required_company_cols].rename(
                        columns={'dst': 'telephone_number'})
                    df_company.to_csv(
                        f'{DATA_DIR}/df_company__{current_date}.csv', index=False)
                    logger.info(f'Company data saved with {
                                len(df_company)} records')
                else:
                    logger.warning(f"Missing company columns. Available columns: {
                                   df.columns.tolist()}")
...
if __name__ == "__main__":
    try:
        fetch_data_from_api(
            config['URL'], config['API_KEY'], config.LAST_ID, config['LIMIT'])
    except Exception as e:
