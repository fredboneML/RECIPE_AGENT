...
def create_db_engine():
    """Create and verify database connection with retries"""
    max_retries = 5
    retry_delay = 10
...
def safely_read_csv(file_path, max_retries=3, retry_delay=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"Attempt {attempt + 1} to read {file_path} failed: {e}. Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:
            ...
            try:
                if '__' in filename and '.csv' in filename:
                    ...
                    if date_part == target_date:
                        ...
                        if os.path.exists(file_path):  # Additional check before deletion
                            os.remove(file_path)
...
def main():
    ...
    try:
        ...
        logger.info(f"Using database: {config['POSTGRES_DB']} on host: {config['DB_HOST']}")
        ...
        try:
            ...
            fetch_data.fetch_data_from_api(config['URL'], config['API_KEY'], last_id, config['LIMIT'])
            
            # Find and sort data files
            df_files = [f for f in os.listdir(DATA_DIR) if f.startswith('df__')]
            df_files = sorted(
                df_files,
